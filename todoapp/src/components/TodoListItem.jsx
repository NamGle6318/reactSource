...
const TodoListItem = ({ todo, onRemove, onToggle }) => {
  const { id, content, finished, importanted } = todo;
  ...
  return (
    <div className="p-4 flex items-center even:bg-gray-200">
      ...
      >
        {finished ? <MdCheckBox /> : <MdCheckBoxOutlineBlank />}

        <div className={`ml-2 flex-1 ${finished ? "line-through text-gray-400" : ""}`}>{content}</div>
      </div>
